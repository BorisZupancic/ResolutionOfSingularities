...
#class LogMultivectorField_affine(MultivectorField):
#
#    def __init__(self, Y: AlgebraicScheme_subscheme_affine, xi)
...
def NewtonPolyhedron_V2(Y: AlgebraicScheme_subscheme_affine, xi: MultivectorField, U: DiffChart):
    p1 = Polyhedron([]) 
    if Y is not None:
        vertices = []
        for f in Y.defining_ideal().gens():
...
def AdmissiblePolyhedron_V2(Y: AlgebraicScheme_subscheme_affine, xi:MultivectorField, U:DiffChart):
    n = Y.ambient_space().ngens()
    Newt_Y, Newt_xi = NewtonPolyhedron_V2(Y,xi, U)
