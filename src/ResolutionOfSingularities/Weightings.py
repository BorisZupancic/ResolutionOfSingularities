# Copyright (C) 2026 Boris Zupancic <boris.zupancic@mail.mcgill.ca>
#
...
class WeightedSubscheme(AlgebraicScheme_subscheme_affine):
    ...
    - ``X`` -- :class:`sage.schemes.affine.affine_space.AffineSpace_generic`, the ambient affine scheme :math:`X`
    - ``parameters`` -- list, a list ``[x1,...,xk]`` of elements of ``X.coordinate_ring()`` representing the defining functions of the weighted subscheme :math:`Z_\bullet = (x_1^{a_1},\dots,x_k^{a_k})`
    - ``invariant`` -- list, a list ``[a1,...,ak]`` of rationals representing the invariant :math:`(a_1,\dots,a_k) = \mathrm{inv}(Z_\bullet)`
